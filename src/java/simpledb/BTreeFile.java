...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		if (f == null) return findLeftMostLeafPage(tid, dirtypages, pid, perm);
		...
		if (pid.pgcateg() == BTreePageId.LEAF) {
			...
			try {
				...
				throw new DbException("Error pid type!");
			}
			...
			if (!pageIter.hasNext()) {
				throw new DbException("Error in reading entry!");
			}
			BTreeEntry pageEntry = pageIter.next();
			while(true) {
				// if the field to search for is less or equal than entry.filed value, go left
				if (f.compare(Op.LESS_THAN_OR_EQ, pageEntry.getKey())) {
					return findLeafPage(tid, dirtypages, pageEntry.getLeftChild(), perm, f);
				}
				if (pageIter.hasNext()) {
					pageEntry = pageIter.next();
				}
				else {
					break;
				}
			...
			return findLeafPage(tid, dirtypages, pageEntry.getRightChild(), perm, f);

		}
	}

	private BTreeLeafPage findLeftMostLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm)
			throws DbException, TransactionAbortedException{
		if (pid.pgcateg() == BTreePageId.LEAF) {
			return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
		}
			...
			return findLeftMostLeafPage(tid, dirtypages, pageEntry.getLeftChild(), perm);
		}
	}
	/**
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// find the middle key and update the first half to new leaf page
		int num_tuples = page.getNumTuples();
		int middle_idx = num_tuples / 2;
		if (2 * middle_idx < num_tuples)
			middle_idx ++;
		int cnt_tuples = 0;
		...
		Tuple tp = pageIter.next();
		while(true) {
			cnt_tuples ++;
			page.deleteTuple(tp);
			newLeafPage.insertTuple(tp);

			if (cnt_tuples == middle_idx) {
				break;
			}
		...
		// do not forget the neighbour! do it first!
		if (page.getLeftSiblingId() != null) {
			BTreeLeafPage orgLeftSib = (BTreeLeafPage) getPage(tid, dirtypages, page.getLeftSiblingId(),
					Permissions.READ_WRITE);
			orgLeftSib.setRightSiblingId(newLeafPage.getId());
		}

		// update pointer of the two leaf page
		...
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, parentPageId,
				tp.getField(this.keyField));

		// add middle key up into the parent page & update parent pointer
		BTreeEntry e = new BTreeEntry(tp.getField((this.keyField)), newLeafPage.getId(), page.getId());
		parentPage.insertEntry(e);

		// update children pointer
		updateParentPointers(tid, dirtypages, parentPage);

		if (tp.getField(this.keyField).compare(Op.LESS_THAN_OR_EQ, field))
			return page;
		else
        	return newLeafPage;
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage page, Field field) 
					throws DbException, IOException, TransactionAbortedException {
		// some code goes here
		BTreeInternalPage newIterPage = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);

		// find the middle key and update the first half to new internal page
		int num_entries = page.getNumEntries();
//		System.out.println(num_entries);
		int middle_idx = num_entries / 2;
		if (2 * middle_idx < num_entries)
			middle_idx ++;
		int cnt_tuples = 0;
		Iterator<BTreeEntry> pageIter = page.iterator();
		if (!pageIter.hasNext()) {
...
//		BTreeEntry e = pageIter.next();
		...
		// remove this entry from the split page
//		page.deleteKeyAndLeftChild(e);
//		System.out.println(e.getLeftChild());
//		System.out.println(e.getRightChild());
		newIterPage.deleteKeyAndRightChild(e);
//		System.out.println("de"+e.getLeftChild());
//		System.out.println("de"+e.getRightChild());
		// add middle key up into the parent page & update parent pointer
		BTreeEntry adde = new BTreeEntry(e.getKey(), newIterPage.getId(), page.getId());
		parentPage.insertEntry(adde);

		// update children pointer
		updateParentPointers(tid, dirtypages, parentPage);

		// update children of split pages
		updateParentPointers(tid, dirtypages, page);
		updateParentPointers(tid, dirtypages, newIterPage);

		if (e.getKey().compare(Op.LESS_THAN_OR_EQ, field))
			return page;
		else
			return newIterPage;
	}
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int totalSize = page.getNumTuples() + sibling.getNumTuples();
		int halfSize = totalSize / 2;
		int stealSize = halfSize - page.getNumTuples();

		Iterator<Tuple> sibTupIter;
		if (isRightSibling) sibTupIter = sibling.iterator();
		else sibTupIter = sibling.reverseIterator();

		Tuple stealTup = null;
		for (int i = 0; i < stealSize; ++i) {
			if (!sibTupIter.hasNext()) {
				throw new DbException("Not enough tuples to steal!");
			}
			stealTup = sibTupIter.next();
			sibling.deleteTuple(stealTup);
			page.insertTuple(stealTup);
		}

		// pick the first tuple in the right-hand page
		assert stealTup != null;

		Tuple keyTup;
		if (isRightSibling) keyTup = sibTupIter.next();
		else keyTup = stealTup;

		// add this tuple to parent
		entry.setKey(keyTup.getField(this.keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
		int totalSize = page.getNumEntries() + leftSibling.getNumEntries();
		int halfSize = totalSize / 2;
		int stealSize = halfSize - page.getNumEntries();

		Iterator<BTreeEntry> sibEntryIter = leftSibling.reverseIterator();
		BTreeEntry stealEntry = null;

		// add parent entry to right page
		BTreeEntry rightReferEntry = page.iterator().next();
		BTreeEntry leftReferEntry = leftSibling.reverseIterator().next();
		BTreeEntry addEntry = new BTreeEntry(parentEntry.getKey(),leftReferEntry.getRightChild(),
				rightReferEntry.getLeftChild());
		page.insertEntry(addEntry);
		updateParentPointers(tid, dirtypages, page);

		for (int i = 0; i < stealSize; ++i) {
			if (!sibEntryIter.hasNext()) {
				throw new DbException("Not enough entries to steal!");
			}
			stealEntry = sibEntryIter.next();
			leftSibling.deleteKeyAndRightChild(stealEntry);
			if (i != stealSize-1)
				page.insertEntry(stealEntry);
		}

		// pick the first entry in the right-hand page
		assert stealEntry != null;

		// do the rotation
		parentEntry.setKey(stealEntry.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int totalSize = page.getNumEntries() + rightSibling.getNumEntries();
		int halfSize = totalSize / 2;
		int stealSize = halfSize - page.getNumEntries();

		Iterator<BTreeEntry> sibEntryIter = rightSibling.iterator();
		BTreeEntry stealEntry = null;

		// add parent entry to right page
		BTreeEntry rightReferEntry = rightSibling.iterator().next();
		BTreeEntry leftReferEntry = page.reverseIterator().next();
		BTreeEntry addEntry = new BTreeEntry(parentEntry.getKey(),leftReferEntry.getRightChild(),
				rightReferEntry.getLeftChild());
		page.insertEntry(addEntry);
		updateParentPointers(tid, dirtypages, page);
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int totalSize = rightPage.getNumTuples();
		Iterator<Tuple> pageIter = rightPage.iterator();
		Tuple mergeTuple;
		for (int i = 0; i < totalSize; ++i) {
			if (!pageIter.hasNext()) {
				throw new DbException("Not enough tuples to merge!");
			}
			mergeTuple = pageIter.next();
			rightPage.deleteTuple(mergeTuple);
			leftPage.insertTuple(mergeTuple);
		}

		// do not forget neighbours
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage orgRightSib = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(),
					Permissions.READ_WRITE);
			orgRightSib.setLeftSiblingId(leftPage.getId());
		}
		leftPage.setRightSiblingId(rightPage.getRightSiblingId());

		// call setEmptyPage() on deleted pages to make them available for reuse
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		int totalSize = rightPage.getNumEntries();
		Iterator<BTreeEntry> pageIter = rightPage.iterator();
		BTreeEntry mergeEntry;
		BTreeEntry leftReferEntry = leftPage.reverseIterator().next();
		BTreeEntry rightReferEntry = rightPage.iterator().next();
		BTreeEntry addEntry = new BTreeEntry(parentEntry.getKey(), leftReferEntry.getRightChild(),
				 rightReferEntry.getLeftChild());
		leftPage.insertEntry(addEntry);
		...
		for (int i = 0; i < totalSize; ++i) {
			...
			mergeEntry = pageIter.next();

			rightPage.deleteKeyAndLeftChild(mergeEntry);
			leftPage.insertEntry(mergeEntry);
		}
